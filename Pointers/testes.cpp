...
void test_qual_tamanho(){
    t.open("qual_tamanho",3);
    int vet1[]={1};
    int vet2[]={1,3,4,5};
    int vet3[]={1,3,4,5,6,3,2,1,1};
    t.add(qual_tamanho(&vet1[0],&vet1[0]) == 1);
    t.add(qual_tamanho(&vet2[0],&vet2[3]) == 4);
    t.add(qual_tamanho(&vet3[0],&vet3[8]) == 9);
    t.close();
}
...
void test_strclone(){
    t.open("strclone",2);
    {
        ...
        t.add( (aux != a) && (strcmp(a,aux) == 0 )&& tam == 9);
    }
    {
        char a[]="Grifinore";
        int tam;
        char *aux=strclone(a,&tam);
        t.add( (aux != a) && (strcmp(a,aux) == 0 )&& tam == 10);
    }
    t.close();
}
...
void test_newcat(){
    t.open("newcat",4);
    {
        ...
        t.add((conc != c) && (strcmp(c,conc) == 0 ) && tam == 24);
    }
    {
        char a[]="espectro ";
        char b[]="patrono";
        ...
        int tam;
        char *conc=newcat(a,b,&tam);
        t.add((conc != c) && (strcmp(c,conc) == 0 ) && tam == 17);
    }
    t.close();
}
...
void test_distancia_pontos(){
    t.open("distancia_pontos",4);
    Ponto p={13,14};
    Ponto q={20,35};
    t.add(fabs(distancia_pontos(&p,&q) - 22.1359) < 1.0e-4);
    q.x=45;q.y=11;
    t.add(fabs(distancia_pontos(&p,&q) - 32.1403) < 1.0e-4);
    t.close();
}

void test_maior_distancia(){
...
void test_sort_string_maius_minus(){
...
int main(){
    ...
    test_newcat();
    test_subst();
    test_new_vetor_menores();
    test_set_ponto();
    test_swap_ponto();
    test_distancia_pontos();
    test_maior_distancia();
    test_zerar_quad();
    test_quais_quadrantes();
    test_contar_localizando();
    test_cria_ponto();
    test_cria_vetor_pontos();
    test_cria_aluno();
    test_soma_idade_alunos();
    test_aluno_mais_velho();
    test_novo_poligono();
    test_sort_string();
    test_sort_string_maius_minus();
    test_find_menor();
    test_find_extremos();
